export default class CreateGraph {
  ...
  getNodesAndEdges(courseList) {
    ...
    for (let i = 0; i < courseList.length; i++) {
      ...
      let element = this.convertToNode(courseList[i]);
      nodes.push(element);
      ...
      for (let i = 0; i < preReq.length; i++) {
        if (preReq[0] != '') {
          //If prereq exists
          let edge = this.getEdge(preReq[i], curNode);
          edges.push(edge);
        }
      }
    }
  ...
  //Read major 1
  async generateMajor1() {
    var major1 = await this.readLocalFile('major1.txt');
    //Store majors in arrays
    let coursesList1 = this.courseList(major1);

    //Store nodes and edges in arrays
    let Course1NE = this.getNodesAndEdges(coursesList1);
